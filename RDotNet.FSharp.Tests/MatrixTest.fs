...
//   3  6  9  12
[<Test>]
let ``can slice matrix rows from start to specific index`` () =
   let matrix =
      let engine = REngine.GetInstanceFromID (engineName)
      engine.Evaluate ("""matrix(1:12, 3, 4)""") |> S.AsIntegerMatrix
   let actual = matrix.[..1, *]
...
// | 3  6  9  12 |
[<Test>]
let ``can slice matrix rows from specific index to end`` () =
   let matrix =
      let engine = REngine.GetInstanceFromID (engineName)
      engine.Evaluate ("""matrix(1:12, 3, 4)""") |> S.AsIntegerMatrix
   let actual = matrix.[1.., *]
...
//   3  6  9  12
[<Test>]
let ``can slice matrix rows from specific index to specific index`` () =
   let matrix =
      let engine = REngine.GetInstanceFromID (engineName)
      engine.Evaluate ("""matrix(1:12, 3, 4)""") |> S.AsIntegerMatrix
...
//   3  6  9  12
//[<Test>]
//let ``can slice matrix specific row`` () =
//   let matrix =
//      let engine = REngine.GetInstanceFromID (engineName)
//      engine.Evaluate ("""matrix(1:12, 3, 4)""") |> S.AsIntegerMatrix
//   let actual = matrix.[1, *]
...
// --------+
[<Test>]
let ``can slice matrix columns from start to specific index`` () =
   let matrix =
      let engine = REngine.GetInstanceFromID (engineName)
      engine.Evaluate ("""matrix(1:12, 3, 4)""") |> S.AsIntegerMatrix
   let actual = matrix.[*, ..2]
...
//      +------
[<Test>]
let ``can slice matrix columns from specific index to end`` () =
   let matrix =
      let engine = REngine.GetInstanceFromID (engineName)
      engine.Evaluate ("""matrix(1:12, 3, 4)""") |> S.AsIntegerMatrix
   let actual = matrix.[*, 2..]
...
//   +------+
[<Test>]
let ``can slice matrix columns from specific index to specific index`` () =
   let matrix =
      let engine = REngine.GetInstanceFromID (engineName)
      engine.Evaluate ("""matrix(1:12, 3, 4)""") |> S.AsIntegerMatrix
...
//   +---+
//[<Test>]
//let ``can slice matrix specific column`` () =
//   let matrix =
//      let engine = REngine.GetInstanceFromID (engineName)
//      engine.Evaluate ("""matrix(1:12, 3, 4)""") |> S.AsIntegerMatrix
//   let actual = matrix.[*, 1]
